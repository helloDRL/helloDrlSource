...
# =========================================================== #
model_weight_path = "./save_model/T0000_000000/dqn_weight_000.h5"
TRY_LIMIT = 3
...
class DQNEvaluator(DQNCustomClient):
    ...
    def run_model(self, try_limit):
        ...
        while not finish:
            ...
            progress = self.airsim_env.get_progress(car_current_state, self.way_points, check_point_index)
            # print("current progress:{}".format(progress))
            if frozen:
                done = 99  # abnormal status
            elif self.airsim_env.get_speed(car_next_state) <= 1:
                done = 1
            elif collided:
                done = 2
        ...
        print("All evaluation ended.")
        if len(complete_laps) > 0:
            print("The best lap time is [{} mins. , {} secs.] ".format(self.convert_to_minsec(np.min(complete_laps))))
        else:
