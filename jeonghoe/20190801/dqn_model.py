...
class DQNAgent:
    def __init__(self, state_size, action_size, dqn_param):
        ...
        #추가
        self.curr_check_point_index = 0
        self.max_check_point_index = 0
        self.max_check_point_count = 0
        self.curr_progress = 0
        self.max_progress = 0
        self.epsilon_arr = []

        for i in range(100):
            self.epsilon_arr.append(0.1)
    ...
    def get_action(self, state):
        ...
        #self.print_state(state, curr_epsilon)
        #print("Current Progress: {} , Epsilon: {}".format( self.curr_progress, curr_epsilon))
        self.change_epsilon()
    ...
    def get_epsilon(self):
        return self.epsilon_arr[self.curr_progress]

    def change_epsilon(self):
        curr_epsilon = self.epsilon_arr[self.curr_progress]

        if curr_epsilon > self.epsilon_min:
            curr_epsilon *= self.epsilon_decay
            if curr_epsilon<self.epsilon_min:
                curr_epsilon = self.epsilon_min

        self.epsilon_arr[self.curr_progress] = curr_epsilon
    
    def print_state(self,input_state, epsilon):
        state = input_state[0]
        #print("State: {}, Epsilon: {}".format(state, epsilon) )
        print("Forward: [{},{},{},{}], Sign: {}, Moving: {}, Center_Distance: {}, Epsilon: {}, Opstacle: [({},{}) , ({},{})]".format(state[0], state[1],state[2],state[3],state[4],state[5],state[6],epsilon,state[8],state[9],state[10], state[11]))
...
class DQNClient:
    ...
    def run(self, time_limit_hour):
        ...
        while not finish:
            ...
            #추가 
            self.agent.curr_check_point_index = check_point_index
            ...
            self.agent.curr_progress = int(progress)
            if self.agent.curr_progress <0:
                self.agent.curr_progress = 0
            elif self.agent.curr_progress>100:
                self.agent.curr_progress = 100

            if self.agent.max_progress < self.agent.curr_progress:
                self.agent.max_progress = self.agent.curr_progress
            ...
            if done:
                ...
                #추가
                if self.agent.max_check_point_index < check_point_index:
                    self.agent.max_check_point_index = check_point_index
                    self.agent.max_check_point_count = 0
                else:
                    self.agent.max_check_point_count += 1
                ...
                print("Num of steps done :", current_episode, "  score:", round(score,2),
                      "  memory length:",
                      len(self.agent.memory), " Progress:", progress," Max Progress:",self.agent.max_progress ,"  epsilon:", self.agent.get_epsilon())

                if (current_episode % 10 == 0) or (score>self.best_score) or progress > 45:
